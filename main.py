    ...
    def __init__(self, dataset_path, split = 'train'):
            ...
            for subset_name in os.listdir(dataset_path):
                if subset_name.startswith('tr'):
                    ...
                    # 划分训练集、验证集和测试集
                    y_list_train.extend(y_list[:train_size])
                    y_list_dev.extend(y_list[train_size:train_size + dev_size])
                    y_list_test.extend(y_list[train_size + dev_size:])

                    path_list_train.extend(path_list[:train_size])
                    path_list_dev.extend(path_list[train_size:train_size + dev_size])
                    path_list_test.extend(path_list[train_size + dev_size:])

            i = 1
            for subset_name in os.listdir(dataset_path):
                if not subset_name.startswith('tr'):
                    # print(subset_name)
                    path_list =  []
                    y_list = []
                    subset_path = os.path.join(dataset_path,subset_name)
